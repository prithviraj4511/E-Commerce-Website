...
public class AuthController 
{
	...
	public ResponseEntity<?> validateUserCreateToken(@RequestBody @Valid AuthRequest request) {
		...
		try 
		{
			...
			return ResponseEntity.ok(new AuthResp("Auth successful!", utils.generateJwtToken(authenticatedDetails), userService.getUserDetails(request.getEmail())));
		}
		catch (BadCredentialsException e)
		{
			
			System.out.println("err "+e);
			return ResponseEntity.status(HttpStatus.OK).body(new ErrorResponse("error", e.getMessage()));
		}
